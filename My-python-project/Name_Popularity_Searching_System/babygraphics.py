...
def get_x_coordinate(width, year_index):
    ...
    """
    blank_space = (width-GRAPH_MARGIN_SIZE*2) // len(YEARS)
    x = GRAPH_MARGIN_SIZE+year_index * blank_space
    return int(x)
    ...
    for i in range(len(lookup_names)):
        ...
        for j in range(len(YEARS)):
            ...
            if year in name_data[name]:
                ...
                y = GRAPH_MARGIN_SIZE + (CANVAS_HEIGHT-GRAPH_MARGIN_SIZE*2)/MAX_RANK * rank
                label = f"{name} {rank}"
                canvas.create_text(x + TEXT_DX, y, text=label, anchor=tkinter.SW, font='times 10', fill=color)
